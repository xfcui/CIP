...
from CIP import dataset, commons, losses, models
import numpy as np
import pandas as pd
    ...
    def train(self, verbose=1, repeat_index=1):
        ...
        for epoch in range(num_epochs):
            ...
            if val_matric[-1] < best_matric:
                ...
                test_rmse1, test_mae1, tesr_sd1, test_pearson1 = self.evaluate('csar', verbose=1, logger=self.logger)
                self.save(self.config.train.save_path, f'{repeat_index}kkknot_best_valid_{epoch + start_epoch}', val_list)
            else:
                early_stop += 1
                test_rmse, test_mae, tesr_sd, test_pearson = self.evaluate('test', verbose=1, logger=self.logger)
                test_rmse1, test_mae1, tesr_sd1, test_pearson1 = self.evaluate('csar', verbose=1, logger=self.logger)
